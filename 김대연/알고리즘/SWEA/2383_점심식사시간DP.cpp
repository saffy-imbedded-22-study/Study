...
int solve() {
    ...
    for (int i = 0;i < GroupA.size();i++) {
        if (i < 3) {    // 처음 셋은 거리 + 계단높이 + 1 (대기시간) 저장
            DATA[i] = GroupA[i] + stairs[1].height + 1;
        }
        else {
            if (DATA[i - 3] > GroupA[i]) {  // 3번 전 사람의 나가는 시간이 지금 사람의 도착 시간보다 클 경우
                DATA[i] = DATA[i - 3] + stairs[1].height;   // 3번 전 사람이 나가는 시간 + 계단 높이
            }
            else {
                DATA[i] = GroupA[i] + stairs[1].height + 1; // 계단이 비어있으므로 거리 + 계단높이 + 1 (대기시간)
            }
        }
    }
